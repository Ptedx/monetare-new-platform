...
function RatingCard({ title, value, color }) {
...
function StatusBadge({ status }) {
    ...
    if (status === 'PENDENTE' || status === 'A VENCER') classes = "bg-yellow-400 text-white";
...
function ScoreCircle({ value, max, label, color }) {
    // Adjusted sizing to prevent cutoff
    const size = 60;
    const strokeWidth = 5;
    ...
    return (
        <Card className="p-4 flex flex-col items-start justify-between relative overflow-visible">
            ...
            <div className="flex items-center gap-4 w-full">
                ...
                {/* Increased container size and used explicit viewBox */}
...
function TimelineStep({ label, date, status, isLast }) {
    ...
    return (
        <div className="p-8 max-w-[1600px] mx-auto min-h-screen bg-transparent">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                ...
                {/* --- TAB CONTENT IMPLEMENTATIONS (Assinaturas, Documentação, Garantias, Projeto, Perfil) --- */}
